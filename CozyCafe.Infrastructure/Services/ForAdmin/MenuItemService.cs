...
{
    /// <summary>
    /// Сервіс для роботи з елементами меню в адміністративній частині.
    /// Наслідує базовий сервіс, додає кешування, логування і очищення кешу після CRUD.
    /// </summary>
    public class MenuItemService : Service<MenuItem>, IMenuItemService
    {
        ...
        // Список ключів, які ми кешуємо — щоб потім легко очистити
        ...
        public async Task<IEnumerable<MenuItemDto>> GetFilteredAsync(MenuItemFilterModel filterModel)
        {
            ...
            if (!_cache.TryGetValue(cacheKey, out IEnumerable<MenuItemDto> cachedItems))
            {
                ...
                _cacheKeys.Add(cacheKey); // запам'ятовуємо, щоб потім очистити
